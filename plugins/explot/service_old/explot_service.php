...
for($t=0;$t<count($geneids_array);$t++){
	...
	if($tdb=="explorer_aspleaf_potra_v11"){
	$pattern = '/^[a-zA-Z0-9]+[.]+[a-zA-Z0-9]+[.]+[0-9]?[0-9]$/';
	$geneids_array[$t] =$geneids_array[$t];
	$resultprobeset = mysqli_query($GLOBALS["genelist_connection"],"SELECT sample,id,{$logvalue} as log2 FROM $d_source WHERE id='$geneids_array[$t]' $sorting_fild;")	or die(mysqli_error());	
	}else{
	$sorting_fild="  group by sample order by sample";
	$pattern = '/^[a-zA-Z0-9]+[.]+[a-zA-Z0-9]+[.]+[0-9]?[0-9]$/';
