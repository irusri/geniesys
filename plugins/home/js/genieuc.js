...
(function (exports) {
    var genieuc = function (config) {
		...
		// Upload check, can be used for sharding
		this.upcheck = function (md5) {
            ...
            xhrs.open('POST', this.checkurl, false); // Synchronize
            if (this.headers) {
        ...
        this.credom = function () {
            let up_id = 'genieuc_' + this.id + '_' + new Date().getTime();
        ...
        this.computeTime = function (totalTime) {
            if (totalTime < 1000) {
                totalTime = (totalTime / 1000).toFixed(4) + 'seconds';
            } else {
        ...
        this.result = function (xhr) {
            ...
            if ((xhr.readyState == 4) && (xhr.status == 200)) {
                ...
                if (typeof that.progress == 'function') {
                    that.progress(that.progress_num, other,that);
                }
		...
		this.startUpload = function(){
         //   that.upcheck(that.md5str); // Check upload
    ...
    exports.genieuc = genieuc;
})(this);
// spark-md5 file is used to calculate file md5 value
