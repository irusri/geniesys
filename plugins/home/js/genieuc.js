var genieuc_upload = {
    genieuc: function (config) {
        ...
        if (jQuery.upId && jQuery.upUrl) {
            ...
            jQuery.genieuc_addFileInput();
        }
    },
    genieuc_limitFileSize: function (file, limitSize) {
        var arr = ["KB", "MB", "GB"],
    ...
    genieuc_getPercent: function (num, total) {
        num = parseFloat(num);
    ...
    genieuc_addFileInput: function () {
        ...
        if (C) {
            html += '<input type="file" id="' + jQuery.upInputId + '" class="' + C + '" onchange="jQuery.genieuc_upload()" style="position:absolute;left:' + Y + 'px;top:' + X + 'px;opacity:0;z-index:9999;width:' + W + 'px;height:' + H + 'px;">';
        } else {
            html += '<input type="file" id="' + jQuery.upInputId + '" onchange="jQuery.genieuc_upload()" style="position:absolute;left:' + Y + 'px;top:' + X + 'px;opacity:0;z-index:9999;width:' + W + 'px;height:' + H + 'px;">';
        }
        jQuery('#' + jQuery.upId).html(html);
    },
    genieuc_upFileInput: function () {
        var X = jQuery('#' + jQuery.upId).position().top;
        var Y = jQuery('#' + jQuery.upId).position().left;
        var W = jQuery('#' + jQuery.upId).innerWidth();
        var H = jQuery('#' + jQuery.upId).innerHeight();
    ...
    genieuc_upload: function () {
        ...
        if (jQuery.upType) {
            ...
            if (jQuery.inArray(suffix, uptype) == -1) {
                jQuery.upError = 'File types not allowed to upload-' + suffix;
            }
        }
        if (jQuery.upMaxSize) {
            if (!jQuery.genieuc_limitFileSize(file, jQuery.upMaxSize + 'MB')) {
                jQuery.upError = 'Uploaded file is too large';
            }
        }
        ...
        var md5taskid = window.setInterval(function () {
            if (jQuery.fileMD5) {
                setTimeout("jQuery.genieuc_upload_core()", "100");
                window.clearInterval(md5taskid);
            }
        }, 100);
    },
    genieuc_upload_core: function () {
        var file = jQuery.tempFile;
        if (!file) {
            return false;
        }
        ...
        setInterval("jQuery.genieuc_upFileInput();", 500);
        var re = [];
        var start, end = 0;
            ...
            if (stack[i2]) {
                ...
                xhr.onreadystatechange = function () {
                    ...
                    if (xhr.readyState == 4 && xhr.status == 200) {
                        ...
                        ++succeed;
                        var cent = jQuery.genieuc_getPercent(succeed, shardCount);
...
/*
 * spark-md5 Used to calculate file md5 value, this is an external class
 */
